"""Thin (x) -> y wrappers around torch_structured primitives plus a make_linear
factory used by the recurrent POC benchmarks.
...
def make_linear(
    kind: str, in_features: int, out_features: int, bias: bool = True, **kwargs
) -> nn.Module:
...
if __name__ == "__main__":
    x = torch.randn(2, 64)
    for k in _SUPPORTED:
        m = make_linear(k, 64, 64)
        y = m(x)
        print(f"{k}: in={tuple(x.shape)} -> out={tuple(y.shape)}")
