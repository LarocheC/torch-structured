...
class Butterfly(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if self.init is None or self.init == 'empty':
            ...
            # Deferred import to avoid circular import at package init time.
            from . import special as _special
            special_fn = (_special.fft if self.init == 'fft_no_br' else _special.ifft)
    ...
    def diagonal_multiply_(self, diagonal, diag_first):
        # Deferred import to avoid circular import at package init time.
        from .combine import diagonal_butterfly
        return diagonal_butterfly(self, diagonal, diag_first, inplace=True)

    def to_base4(self):
        # Deferred import to avoid circular import at package init time.
        from .butterfly_base4 import ButterflyBase4
        with torch.no_grad():
