...
from torch_structured.permutation import perm_vec_to_mat, invert, matrix_to_butterfly_factor
    ...
    def test_modular_balance(self):
        ...
        for n in [2, 16, 64]:
            for _ in range(num_repeats):
                ...
                Rinv_perms, L_vec = torch_structured.permutation.modular_balance(v)
                self.assertTrue(torch_structured.permutation.is_modular_balanced(L_vec))
                v2 = v
                for p in Rinv_perms:
                    v2 = v2[p]
                ...
                for p in reversed(Rinv_perms):
                    lv2 = lv2[torch_structured.permutation.invert(p)]
        ...
        for n in [2, 13, 38]:
            for increasing_stride in [False, True]:
                for complex in [False, True]:
                    for _ in range(num_repeats):
                        v = torch.randperm(n)
                        b = torch_structured.permutation.perm2butterfly_slow(v, complex,
                                                                            increasing_stride)
        ...
        for n in [2, 13, 38]:
            for increasing_stride in [False, True]:
                for complex in [False, True]:
                    for _ in range(num_repeats):
                        v = torch.randperm(n)
                        b = torch_structured.permutation.perm2butterfly(v, complex,
                                                                       increasing_stride)
                        input = torch.arange(n, dtype=torch.float32)
                        if complex:
                            input = input.to(torch.complex64)
